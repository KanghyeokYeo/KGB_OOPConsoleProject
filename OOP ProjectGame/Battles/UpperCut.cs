...
{
    public class UpperCut : Battle
    {
        private string input1;
        private string input2;
        public UpperCut(Game game) : base(game) { }
        public override void Enter()
        {
            Console.Clear();
            Console.WriteLine("UpperCut이 날라옵니다");
            Console.WriteLine("회피를 하려면 1을 입력하시오");
            input1 = Console.ReadLine();
            Thread.Sleep(3000);
            if (input1 != "1")
            {
                Console.WriteLine("회피를 실패하여 30 데미지 받았습니다");
                game.player.curHp -= 30;
